...
- **What it is:** Enough is an iOS app that teaches people just enough of a language for a
  short trip — a weekend away or a week's holiday — using spaced-repetition flashcard mechanics
  (in the spirit of Anki) under the hood. Users buy language decks (roughly £3 each), each
  scoped to practical, trip-ready vocabulary and phrases rather than a full course.
- **What you're building:** see `README.md` and (if present) `PLAN.md` or
...
- **The `README.md` is a product document** — what the project IS and how a human uses it. It is
  **maintainer-owned**: the loop NEVER edits it, and it does **not** track implementation status or
  the backlog (that lives in `TASKS.json` + the dashboard, always current — a status section in the
  README would only go stale).
...
5. If working BY HAND (no loop running), set the task's `TASKS.json` `"status"` in the same commit —
   ...
   `README.md` as something to update per change — it is maintainer-owned product documentation, not
   a status log (golden rule 3).
...
- Stack: **iOS / Swift**, project generated via **XcodeGen** (`project.yml` → `Enough.xcodeproj`,
  git-ignored, regenerated on every build). Definition-of-Done commands, mirrored verbatim between
  `.github/workflows/ci.yml` and local runs:
  - **Format**: `swift-format lint --recursive --strict Sources Tests` (adjust once `T001` picks
...
- Define the exact, final versions of these commands in
  [`.harness/docs/HARNESS.md`](./.harness/docs/HARNESS.md) §5 (via the `.harness/custom/docs/HARNESS.md`
  overlay) once `T001` has picked real tool versions/configs, and mirror them verbatim in
...
- **Your tooling must not sweep the vendored harness tree.** The repo vendors `.harness/**`
  ...
  `README.md`), none of it written to your formatter/linter's rules. Exclude `.harness/` from
  `swiftlint`'s `excluded:` list and keep it out of any Swift source-discovery glob. Any task that
  reconfigures a whole-repo tool must keep that exclusion intact.
- Before pushing, the code should pass that full suite locally — it mirrors CI exactly.
- Tasks marked **🔒 needs-human** require the user (credentials, provisioning, App Store Connect /
  StoreKit setup, anything spending real money or touching production). Do not attempt the
  human-gated portion yourself; prepare everything around it and hand off.
